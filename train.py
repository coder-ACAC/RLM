...
def train(args, data):
    feature_encoder, train_data, valid_data, test_data, all_fea_np, neighbor_info = data
    ...
    model = GNN(args, feature_len, all_fea_th, neighbor_info)
    MemoryNet = MNet(args)
    ...
    for i in range(args.epoch):
        ...
        val_mrr, log_eval = evaluate(model, MemoryNet, 'valid', valid_data, args)
    ...
    evaluate(model, MemoryNet, 'test', test_data, args)

    if args.save_model:
        if not os.path.exists('../saved/'):
            os.mkdir('../saved/')
        directory = '../saved/%s' % (args.dataset)
        if not os.path.exists(directory):
            os.mkdir(directory)
        torch.save(best_model_params, directory + '/model.pt')
        with open(directory + '/hparams.pkl', 'wb') as f:
            hp_dict = {'gnn': args.gnn, 'layer': args.layer, 'feature_len': feature_len, 'dim_atom': args.dim_atom, 'dim_graph': args.dim_graph}
...
def equivalence_cl_loss(reactant_embeddings, product_embeddings, MemoryNet, args):
    ...
    # Choice 1
    # dist = torch.cdist(reactant_embeddings + relation_vec, product_embeddings, p=2)
    # pos = torch.diag(dist)
